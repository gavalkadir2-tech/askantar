...
export function ReportsTab({ farmers, purchases, sales, buyers, expenses, personnel, vehicles, personnelAttendance, personnelPayments, payments, buyerPayments, settings }) {
  ...
  const totalPaymentsOut = filteredPayments.reduce((s, p) => s + p.amount, 0);
  const totalCollectionsIn = filteredBuyerPayments.reduce((s, p) => s + p.amount, 0);
  ...
  const exportExcel = () => {
    ...
    const paymentRows = filteredPayments.map((p) => {
      const f = farmers.find((x) => x.id === p.farmerId);
      return { 'Tarih': p.date, 'Çiftçi': f ? f.name : '', 'Tür': p.payType === 'avans' ? 'Avans' : 'Ödeme', 'Tutar': p.amount, 'Not': p.note || '' };
    });
    const wsPayments = XLSX.utils.json_to_sheet(paymentRows);
    XLSX.utils.book_append_sheet(wb, wsPayments, 'Odemeler (Ciftciye)');

    const collectionRows = filteredBuyerPayments.map((p) => {
      const b = buyers.find((x) => x.id === p.buyerId);
      return { 'Tarih': p.date, 'Alıcı': b ? b.name : '', 'Tutar': p.amount, 'Not': p.note || '' };
    });
  ...
  return (
    <div>
      ...
      <div className="zk-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px,1fr))', marginBottom: 16 }}>
        ...
        <StatCard label="Çiftçiye yapılan ödeme" value={fmtTL(totalPaymentsOut)} tone={COLORS.gold} icon={Wallet} />
        <StatCard label="Alıcıdan tahsilat" value={fmtTL(totalCollectionsIn)} tone={COLORS.blue} icon={Wallet} />
