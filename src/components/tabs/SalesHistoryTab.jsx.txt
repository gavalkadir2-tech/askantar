...
import { fmtDate, fmtKg, fmtTL, storageSet, todayStr } from '../../lib/format';
...
export function SalesHistoryTab({ buyers, sales, setSales, settings, onPrintSaleReceipt, vehicles, bankAccounts }) {
  ...
  const exportFilteredExcel = () => {
    const rows = filtered.map((s) => ({
      'Makbuz No': s.makbuzNo || '', 'Tarih': s.date, 'Alıcı': s.buyerName, 'Sınıf': s.grade || '',
      'Kg': s.kg, 'Kg fiyatı': s.pricePerKg, 'Tutar': s.amount, 'Araç': s.vehiclePlaka || '',
      'Vade tarihi': s.vadeTarihi || '', 'Not': s.note || '',
    }));
    const ws = XLSX.utils.json_to_sheet(rows);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Satis Gecmisi');
    XLSX.writeFile(wb, `satis-gecmisi-${todayStr()}.xlsx`);
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10 }}>
        ...
        <button className="zk-btn zk-btn-blue" onClick={exportFilteredExcel} disabled={filtered.length === 0}><Download size={13} /> Excel'e aktar</button>
      </div>
