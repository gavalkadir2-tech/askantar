...
export function VoiceAssistant({ farmers, setFarmers, priceList, purchases, setPurchases, payments, setPayments, expenses, setExpenses, reminders, setReminders, settings, buyers, sales, setSales, buyerPayments, setBuyerPayments }) {
  ...
  const [messages, setMessages] = useState([
    { role: 'assistant', text: 'Merhaba! Alım, satış, çiftçi ekleme, ödeme/avans, tahsilat, gider ve hatırlatma gibi işlemleri sesli veya yazarak yapabilirsiniz. Bakiye de sorabilirsiniz. Örnek: "Mehmet\'ten 50 kilo Tirilye 100 liradan al", "Ayşe\'ye 200 kilo 120 liradan sat", "Ahmet\'in bakiyesi ne kadar?"' },
  ]);
  ...
  const handleCommand = async (text) => {
    ...
    const ctx = { farmers, priceList, settings, buyers: buyers || [], purchases, payments, sales: sales || [], buyerPayments: buyerPayments || [] };
    ...
    if (result.ok && result.type === 'query') {
      setPending(null);
      setAmbiguity(null);
      setMessages((m) => [...m, { role: 'assistant', text: `${result.viaAi ? '✨ ' : ''}${result.answer}` }]);
      return;
    }
  ...
  const resolveAmbiguity = (chosenEntity) => {
    ...
    if (!result || (result.type === 'purchase' && (!result.kg || !result.price || !result.varietyLabel)) || (result.type === 'sale' && (!result.kg || !result.price))) {
      setMessages((m) => [...m, { role: 'assistant', text: `${chosenEntity.name} seçildi ama komutun geri kalanını tam anlayamadım, lütfen tekrar söyleyin.` }]);
      return;
    }
    setPending(result);
    setMessages((m) => [...m, { role: 'assistant', text: `Anladığım: ${pendingSummaryText(result)} Kaydedeyim mi?` }]);
  };
  ...
  const confirmSave = async () => {
    ...
    if (pending.type === 'purchase') {
      ...
      const record = {
        ...
        amount, netPayment: amount, note: 'Sesli komutla eklendi', vadeTarihi: pending.vadeTarihi || null, createdAt: Date.now(),
      };
    ...
    if (pending.type === 'sale') {
      const amount = pending.kg * pending.price;
      const record = {
        id: uid(), makbuzNo: nextReceiptNo(sales || [], settings?.salesReceiptNext), buyerId: pending.buyer.id,
        date: todayStr(), grade: pending.varietyLabel || '', kg: pending.kg, pricePerKg: pending.price, amount,
        note: 'Sesli komutla eklendi', vehicleId: null, vehiclePlaka: '', paymentMethod: 'nakit', bankAccountId: null,
        vadeTarihi: pending.vadeTarihi || null, createdAt: Date.now(),
      };
      const next = [...(sales || []), record];
      setSales(next);
      await storageSet('zk:sales', next);
      setMessages((m) => [...m, { role: 'assistant', text: `Satış kaydedildi ✓ (Makbuz #${record.makbuzNo})` }]);
    }

    if (pending.type === 'collection') {
      const record = { id: uid(), buyerId: pending.buyer.id, date: todayStr(), amount: pending.amount, note: 'Sesli komutla eklendi', createdAt: Date.now() };
      const next = [...(buyerPayments || []), record];
      setBuyerPayments(next);
      await storageSet('zk:buyerPayments', next);
      setMessages((m) => [...m, { role: 'assistant', text: 'Tahsilat kaydedildi ✓' }]);
    }
